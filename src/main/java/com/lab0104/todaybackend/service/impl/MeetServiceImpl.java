...
public class MeetServiceImpl implements MeetService {
    ...
    public List<MeetDTO.Info> findList(int number, int size){
        ...
        Page<MeetDTO.Info> response = meetRepository.findAll(pageableWithSort).map(dataConversion::meetEntityToDTOList);
    ...
    public List<MeetDTO.Info> findListByCategory(int number, int size, long categoryId){
        ...
        Sort sort = Sort.by(Sort.Direction.DESC, "id");
        Pageable pageableWithSort = PageRequest.of(number, size, sort);
        Page<MeetDTO.Info> response = meetRepository.findByCategory(category, pageableWithSort).map(dataConversion::meetEntityToDTOList);
    ...
    public List<MeetDTO.Info> findListByUser(int number, int size, long userId){
        ...
        Sort sort = Sort.by(Sort.Direction.DESC, "id");
        Pageable pageableWithSort = PageRequest.of(number, size, sort);
        Page<MeetDTO.Info> response = meetRepository.findByUser(user, pageableWithSort).map(dataConversion::meetEntityToDTOList);
